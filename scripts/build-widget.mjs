...
;(function(){
  ...
  function mkOverlay(nom){
    return '<div id="crd-overlay" role="dialog" aria-modal="true">'
      +'<div id="crd-modal">'
      +'<div class="ch"><div><div class="ct">Reserver une table</div><div class="cn">'+nom+'</div></div>'
      ...
      +'<div class="cf2">Propulse par <b>CERYDRA</b></div>'
      +'</div></div>';
  }
  ...
  function mkForm(resto,hs){
    ...
    return '<form id="crd-f" novalidate>'
      +'<div class="g2"><div class="cf"><label>Prenom *</label><input name="prenom" placeholder="Jean" required autocomplete="given-name"/></div>'
      ...
      +'<div class="cf" style="margin-top:0"><label>Telephone *</label><input name="telephone" placeholder="06 12 34 56 78" required/></div></div>'
      ...
      +'<input id="crd-hp" disabled placeholder="Choisissez une date"/>'
  ...
  function mkConfirm(resto,f){
    ...
    return '<div class="ok"><div class="oi"><svg viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg></div>'
      +'<h2>Reservation confirmee !</h2><p>'+(resto.message_confirmation||'Merci '+f.prenom+', a bientot chez '+resto.nom+' !')+'</p>'
      +'<div class="rc">'
  ...
  function bindForm(sh,resto,hs){
    ...
    dateEl.addEventListener('change',function(){
      ...
      if(!s.length){hp.style.display='';hsel.style.display='none';al.className='al aw';al.textContent='Le restaurant est ferme ce jour-la.';al.style.display='block';sb.disabled=true;return;}
      sb.disabled=false;
    ...
    form.addEventListener('submit',function(e){
      ...
      if(!f.heure){al.className='al ae';al.textContent='Veuillez selectionner un horaire.';al.style.display='block';return;}
  ...
  function init(){
    ...
    // Bouton immédiat — pas d'attente réseau
    wrap.innerHTML=mkBtn();

    var cache=null,loaded=false,loading=false;
    ...
    sh.getElementById('crd-btn').addEventListener('click',function(){
      ...
      if(loaded&&cache){
        ...
        sh.getElementById('crd-fa').innerHTML=mkForm(cache.resto,cache.horaires);
        bindForm(sh,cache.resto,cache.horaires);
        return;
      }
      // Premier clic : charge les données
      loading=true;
      ...
      apiGet(slug).then(function(data){
        if(data.error){
          sh.getElementById('crd-fa').innerHTML='<div style="padding:32px 22px;text-align:center;font-family:Inter,system-ui;color:#6b7280;font-size:13px;">'+data.error+'</div>';
        ...
        sh.getElementById('crd-fa').innerHTML='<div style="padding:32px 22px;text-align:center;font-family:Inter,system-ui;color:#dc2626;font-size:13px;">Erreur de connexion.</div>';
        loading=false;
      });
    });
