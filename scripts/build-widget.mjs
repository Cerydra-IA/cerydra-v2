/**
 * Lit le .env, injecte les clés Supabase dans le widget, écrit public/widget.js
 */
...
function loadEnv() {
  ...
  try {
    ...
    console.warn('[widget] Pas de .env — utilisation des variables système.')
  }
  return {
...
if (!url || !key) {
  console.error('[widget] VITE_SUPABASE_URL ou VITE_SUPABASE_ANON_KEY manquant.')
  process.exit(1)
}

// ── Source du widget ──────────────────────────────────────────────
const src = `/* CERYDRA Widget — généré automatiquement
   Usage: <script src="https://cerydra.fr/widget.js" data-resto="mon-slug"><\/script> */
;(function(){
var SB_URL="${url}",SB_KEY="${key}";
...
console.log('[widget] ✓ public/widget.js généré (' + src.length + ' bytes)')
