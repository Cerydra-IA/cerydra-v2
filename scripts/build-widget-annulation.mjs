/**
 * Génère public/widget-annulation.js — aucune clé Supabase.
 * Le widget appelle /api/annulation (Netlify Function).
 */
...
const src = `/* CERYDRA Widget Annulation — aucune clé sensible.
   Usage: <script src="https://app.cerydra.fr/widget-annulation.js" data-resto="mon-slug"><\/script> */
;(function(){
  var el=document.currentScript||document.querySelector('script[data-resto-annulation],[data-widget="annulation"]');
  if(!el) el=document.querySelector('script[src*="widget-annulation"]');
  ...
  var CSS=\`
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    #can-btn{position:fixed;bottom:28px;left:28px;z-index:2147483646;background:#fff;color:#1a1a2e;border:1.5px solid #e5e7eb;padding:13px 20px;border-radius:50px;font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:7px;box-shadow:0 4px 20px rgba(0,0,0,.1);transition:transform .15s,box-shadow .15s,border-color .15s;font-family:'Inter',system-ui,sans-serif;}
    #can-btn:hover{transform:translateY(-2px);box-shadow:0 8px 28px rgba(0,0,0,.15);border-color:#1a1a2e;}
    #can-btn svg{width:15px;height:15px;flex-shrink:0;}
    ...
    .cf{display:flex;flex-direction:column;gap:5px;margin-top:0;}
  ...
  function mkBtn(){
    return '<button id="can-btn" aria-label="Annuler une reservation">'
      +'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">'
      +'<circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>'
      +'</svg>Annuler ma reservation</button>';
  }

  function mkOverlay(){
    return '<div id="can-overlay" role="dialog" aria-modal="true">'
      +'<div id="can-modal">'
      +'<div class="ch"><div><div class="ct">Annuler une reservation</div><div class="cn" id="can-cn"></div></div>'
  ...
  function bindEmailForm(sh,restoNom){
    ...
    form.addEventListener('submit',function(e){
      ...
      apiGet(email).then(function(data){
        if(data.error){
          showAlert(sh,data.error,'err');
          sb.disabled=false;sb.textContent='Rechercher mes reservations';
          return;
        }
        var fa=sh.getElementById('can-fa');
        fa.innerHTML=mkResaList(data.reservations||[]);
        bindResaList(sh,email,restoNom);
      }).catch(function(err){
        console.error('[CERYDRA Annulation]',err);
        showAlert(sh,'Erreur de connexion. Reessayez.','err');
        sb.disabled=false;sb.textContent='Rechercher mes reservations';
      });
    });
  }

  function bindResaList(sh,email,restoNom){
    var btns=sh.querySelectorAll('.btn-cancel-resa');
    btns.forEach(function(btn){
      btn.addEventListener('click',function(){
        ...
        apiPost(token).then(function(res){
          if(res.success){
            sh.getElementById('can-fa').innerHTML=mkConfirm(nom,date,heure);
          } else {
            // Re-enable button and show error
            btn.disabled=false;btn.textContent='Annuler cette reservation';
            var card=btn.closest('.resa-card');
            var al=document.createElement('div');
            al.className='al ae';al.textContent=res.error||'Erreur. Reessayez.';
  ...
  function init(){
    ...
    function openModal(){
      if(!sh.getElementById('can-overlay')){
        overlayWrap.innerHTML=mkOverlay();
        sh.getElementById('can-x').addEventListener('click',closeModal);
        sh.getElementById('can-overlay').addEventListener('click',function(e){
          if(e.target===sh.getElementById('can-overlay'))closeModal();
        });
      }
      var cn=sh.getElementById('can-cn');
      if(cn&&restoNom)cn.textContent=restoNom;
      sh.getElementById('can-fa').innerHTML=mkEmailForm();
      sh.getElementById('can-overlay').classList.add('open');
      document.body.style.overflow='hidden';
      bindEmailForm(sh,restoNom);
      // Focus email input
      setTimeout(function(){
        var inp=sh.querySelector('input[name="email"]');
        if(inp)inp.focus();
      },200);
    }

    sh.getElementById('can-btn').addEventListener('click',function(){
      if(!loaded){
        // Charge le nom du restaurant la premiere fois
        fetch(API_BASE+'/api/widget?slug='+encodeURIComponent(slug))
          .then(function(r){return r.json();})
          .then(function(d){if(d.resto)restoNom=d.resto.nom;loaded=true;})
          .catch(function(){loaded=true;})
          .finally(function(){openModal();});
      } else {
        openModal();
      }
    });
...
console.log('[widget-annulation] public/widget-annulation.js genere (sans cles Supabase)')
